...
\bibitem{slr_reliability}
  \textit{A Systematic Literature Review on Machine Learning for Network Reliability in Telecommunications Industry}. 

\bibitem{infinite_mixture}
  \textit{An Infinite Multivariate Categorical Mixture Model for Self-Diagnosis of Telecommunication Networks}. 

\bibitem{automated_gpon}
  \textit{Automated management of optical fibers infrastructure in FTTH GPON networks}. 

\bibitem{stacking_ensemble}
  \textit{A Stacking Ensemble ML-Based Failure Prediction Model for Optical Networks with Imbalanced Data}. 

\bibitem{shap_lime}
  \textit{Predicting FTTH Take-Up Ratio in West Java Using SHAP and LIME Methods}. 

\bibitem{nokia_predictive}
  Nokia Corporation. \textit{Predictive Maintenance and Optical Analytics Framework}. 

\bibitem{vc4_predictive}
  VC4 Network Lifecycle Management. \textit{How to Predict a Fiber Break Before It Happens: Math, Models, and Metadata}. 
...
\bibitem{optical_monitoring}
  \textit{Monitoreo Avanzado y Análisis de Datos Estructurados en Redes Ópticas de Próxima Generación}. 
...
\newpage
  \section{Diseño de Arquitectura del Sistema:}
    ...
    \noindent
    El sisguiente diagrama muestra la interacción de los distintos dispositivos con el sistema. Se observa la entrada que recibe el modelo predictivo (Desde las ONT, OLT y NOC) y la 
    salida, en donde realiza una llamada a la API de Zammad para emitir el reporte al equipo de LLA.

    \begin{figure}[H]
            \centering
            \includegraphics[width=0.8\textwidth]{Esquemas e imágenes/Diagrama de contexto.png}
...
\section{Gestión de Riesgos:}
    ...
    \noindent
    El diseño de interfza se centra en la visualización de los resultados del modelo predictivo y distribución de datos. Se propone un sistema de notificación por correo electrónico que entrega
    ...
    que los técnicos y administradores de red reciban información crítica directamente en sus dispositivos móviles o en sus puestos de trabajo en el momento exactor en que se requiera su atención.

    \subsection{Modelo de Navegación}
    \noindent
    En el siguiente diagrama modela el flujo de información desde que se evalúa la anomalía hasta que la alerta es consumida por el usuario. La navegación del sistema opera d ela siguiente manera:
    ...
    La interfaz de usuario final se ha diseñado pensando en la inmediatez, aprovechando la vista móvil de aplicaciones de coreo electrónico para el usuario a través de la integración con Zammad.
    El diseño carece de elementos distractivos y prioriza una jerarquía visual de la información técnica crítica:

    \begin{itemize}
        \item \textbf{Bandeja de entrada:} La alerta se identifica claramente bajo el remitente del sistema de tickets y un asunto ("Alerta predictiva"), destacándose frente a los correos regulares.
        \item \textbf{Cuerpo de la Alerta:} Una vez abierto el correo, la interfaz presenta un resumen en un formato de lista para agilizar la lectura técnica: \begin{itemize}
            \item \textbf{Contexto de la falla:} Se especifica inmediatamente el "Riesgo de degradación FTTH".
            \item \textbf{Ubicación y Hardware:} Se expone la topología afectada, detallando la ciudad (Ej. Colón) y el equipo físico (Ej. Dispositivo: OLT-4; Puerto: PON 0/1/2). Esto es vital para
            orientar rápidamente las acciones en terreno.
            \item \textbf{Métrica de riesgo:} Se destaca la "probabilidad de Falla" mediante un porcentaje claro (Ej. $85\%$) acompañado de un indicador visual que denota severidad de la alerta de forma intuitiva.  
        \end{itemize}

        \item \textbf{Detalles del Análisis:} Una sección dedicada a la explicabilidad del modelo predictivo fundamental para la confianza del usuario. Aquí se desglosa el "Impacto estimado" (Cantidad de ONTs en riesgo ca caída inminente
        ), el patrón recnocido por el modelo y la causa temporal.
    \end{itemize}
